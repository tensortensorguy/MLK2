...
| `backends/` | Tier 0 interpreter backend, CPU backend (KernelModule execution + native AOT artifacts: C++ source, x86-64 assembly, and CUDA emission with opt-in shared-memory slab reuse — the slab value logic is pinned bit-exact through the C++ mirror artifact; out-of-process cc/nvcc build + dlopen driver — docs/polyhedral_spec.md §backend, §GPU-backend) |

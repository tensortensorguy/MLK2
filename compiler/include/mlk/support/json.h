...
public:
    ...
    [[nodiscard]] Type type() const {
        ...
        if (std::holds_alternative<bool>(storage_)) return Type::Bool;
        if (std::holds_alternative<int64_t>(storage_)) return Type::Int;
        if (std::holds_alternative<double>(storage_)) return Type::Double;
        if (std::holds_alternative<std::string>(storage_)) return Type::String;
        if (std::holds_alternative<Array>(storage_)) return Type::Array;
        return Type::Object;
    }
